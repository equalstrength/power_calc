...
Using the package InteractionPoweR (IPR)[@baranger_tutorial_2022] we can use the interaction effect size as a parameter for simulations. It is assumed that the treatment variables are not correlated but this can be changed in the  *r.x1.x2* argument of the function. 
...
Using the R code shared by Edvard Nergård Larsen, it is possible to calculate the power with additional parameters. The estimates result from linear models [@gomila_logistic_2021] of simulated datasets incorporating different combinations of parameters present in the EqualStrength research design. 
...
- There is a large variation of sample sizes depending on the parameters postulated:

| Baseline<br/>call-back| Main <br/> treatment | Second <br/> treatment| Interaction| Min. sample<br/>(80% power)|
|-------:|------:|------:|--------:|:----------|
|     0.3|   -0.3|  -0.30|    0.150|100        |
|     0.3|   -0.3|  -0.30|    0.075|300        |
|     0.3|   -0.3|  -0.15|    0.075|700        |
|     0.3|   -0.3|  -0.15|    0.150|700        |
|     0.5|   -0.3|  -0.30|    0.150|1,300      |
|     0.5|   -0.3|  -0.15|    0.150|1,500      |
|     0.5|   -0.3|  -0.30|    0.075|2,500      |
|     0.5|   -0.3|  -0.15|    0.075|5,900      |
|     0.3|   -0.1|  -0.05|    0.050|11,100     |
|     0.3|   -0.1|  -0.10|    0.050|11,700     |
|     0.5|   -0.1|  -0.05|    0.050|15,300     |
|     0.5|   -0.1|  -0.10|    0.050|15,900     |

## Next steps

1) Decide on the effect size of the interaction term based on previous studies or on what is "substantively important"[@gelman_beyond_2014]/interesting[@lakens_sample_2022].

2) Decide if we should try to generate estimates based on three-way interactions [^2].

3) Discuss feasibility at the country level considering resources available for data collection, expected effect size, as well as the number of new positions available for employment, accommodation, and childcare. 

4) Decide on additional treatment groups for each country considering that each additional group would require a 33% larger sample.  


[^2]: The developers of the package InteractionPoweR indicate that they would like to include 3-way interactions in a future version, but emphasise that "[There are several steps in the analysis where we do not know exactly what to do for 3-way interactions](https://dbaranger.github.io/InteractionPoweR/articles/CommonQuestions.html)".
